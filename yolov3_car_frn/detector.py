...
class Detector(torch.nn.Module):
    ...
    def predict(self,path):
        ...
        with torch.no_grad():
            ...
            out_value = self.forward(img_4, 0.3, cfg.ANCHORS_GROUP_KMEANS)
            ...
            for j in range(2):
                ...
                boxes_ = tool.nms(out_value[classify_mask].cpu())
                print("de102类型",type(boxes_),len(boxes_))
                for i,boxesty in enumerate(boxes_):
                    boxes_to = boxesty.squeeze()
                    boxes_0 = boxes_to[0:1]
                    boxes_1 = boxes_to[1:5]
                    boxes_5 = boxes_to[5:]
                    boxes_to = np.concatenate((boxes_1, boxes_0, boxes_5), axis=0)
                    boxes_rate = boxes_to[0:4] * rate
                    boxes_to = np.concatenate((boxes_rate,boxes_to[4:]), axis=0)
                    # print("boxes_to",boxes_to.tolist())
                    boxes.append(np.array(boxes_to.tolist()))  # 相同类的框做nms，每类做一次，共10类 tool.nms返回torch.stack(r_boxes)，为N，5的形状

                # classify_mask = (out_value[..., -1] == j)  # out_value[..., -1]形状为 N 6, 6为置信度， 坐标， 分类, 取第二维最后的数值（分类）
                # # print(classify_mask.shape)
                # _boxes = out_value[classify_mask]  # 取出相同类的框
                # boxes_draw.append(tool.nms(_boxes.cpu()))  # 相同类的框做nms，每类做一次，共10类 tool.nms返回torch.stack(r_boxes)，为N，5的形状

                # print("116len",len(boxes), boxes)
                # print(len(boxes_draw), boxes_draw)
                # for i, box in enumerate(boxes_draw):
                #     cls_name_list = ["狗", "人"]
                #     try:
                #         img_draw = draw.ImageDraw(img_2)
                #         for n in range(box.shape[0]):
                #             c, x1, y1, x2, y2 = box[n, 0:5] * rate  # N 5
                #             print("cc",c,x1, y1, x2, y2)
